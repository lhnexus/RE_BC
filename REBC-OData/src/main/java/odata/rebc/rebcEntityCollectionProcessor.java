...
public class rebcEntityCollectionProcessor implements EntityCollectionProcessor {

	private static final Logger LOG = LoggerFactory
			.getLogger(EntityCollectionProcessor.class);

	private OData odata;
	private ServiceMetadata srvMetadata;
	private OModel storage;

	public rebcEntityCollectionProcessor(OModel storage) {
	...
	public void readEntityCollection(ODataRequest request,
		...
		EdmEntitySet responseEdmEntitySet = null; // we'll need this to build
													// the ContextURL
		...
		if (segmentCount == 1) { // this is the case for:
			...
			responseEdmEntitySet = startEdmEntitySet; // the response body is
			...
			responseEntityCollection = storage
					.readEntitySetData(startEdmEntitySet, uriInfo.getCustomQueryOptions());
			...
			if (lastSegment instanceof UriResourceNavigation) {
				...
				EdmNavigationProperty edmNavigationProperty = uriResourceNavigation
						.getProperty();
				EdmEntityType targetEntityType = edmNavigationProperty
						.getType();
				// from Categories(1) to Products
				responseEdmEntitySet = REBCUtil.getNavigationTargetEntitySet(
				...
				try {
					responseEntityCollection = storage.getRelatedEntityCollection(
							sourceEntity, targetEntityType);
				} catch (IOException e) {
		...
		if(filterOption != null) {
			...
			try {
			      List<Entity> entityListFilter = returnEntityCollection.getEntities();  
		...
		if (skipOption != null) {
			...
			if (skipNumber >= 0) {
				if (skipNumber <= entityList.size()) {
					...
					entityList = entityList.subList(skipNumber,
							entityList.size());
				} else {
		...
		ContextURL contextUrl = ContextURL.with()
				.entitySet(responseEdmEntitySet).build();
		...
		EntityCollectionSerializerOptions opts = EntityCollectionSerializerOptions
				.with().contextURL(contextUrl).id(id).count(countOption)
				.build();

		EdmEntityType edmEntityType = responseEdmEntitySet.getEntityType();

		SerializerResult serializerResult = serializer.entityCollection(
				this.srvMetadata, edmEntityType, returnEntityCollection, opts);
